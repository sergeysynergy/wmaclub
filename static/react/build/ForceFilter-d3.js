var ForceFilterd3 = React.createClass({displayName: "ForceFilterd3",
    ...
    render: function() {
        ...
        return (
            React.createElement("form", {onSubmit: this.handleSubmit, ref: "forceGraphFilterForm"}, 
                React.createElement("input", {type: "checkbox", className: "btn", value: "zero", onChange: this.handleChange}), "Спрятать вершины без связей" + ' ' +
                "     ", 
                React.createElement("input", {type: "checkbox", className: "btn", value: "radius", onClick: this.handleChange}), "Выделять кол-во связей/атрибутов" + ' ' +
                "     ", 
...
var MySelect = React.createClass({displayName: "MySelect",
    ...
    render: function() {
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            optionsArray.push(
                React.createElement("option", {key: i, value: option.value, onChange: this.handlerChange}, option.display)
            );
        }
        return (
            React.createElement("select", {onChange: this.handlerChange}, optionsArray)
        )
    }
});
...
React.render(
    React.createElement(ForceFilterd3, null),
